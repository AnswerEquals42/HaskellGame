...
-- ** Picture makers
gridP :: Grid (Maybe (NodeInfo Actor)) -> Float -> (Float, Float) -> Picture
gridP (Grid []) z (x', y') = Translate x' y' . Scale z z . Text $ "Empty Grid"
gridP (Grid rows) z (x', y') = 
  ...
  in Translate x' y' 
   . Scale z z 
   . Translate x y 
   . Pictures . mergePictures . zipWith rowP rY $ rows
